...
function showCard() {
  ...
  cardFront.style('background: black; border: 2px solid ' + state.color + ";");
  cardBack.style('background: black; border: 2px solid ' + state.color + ";");
  body.style('background: ' + state.color + ";");


  cardFront.html(state.data[state.card].Q);
  var table = document.getElementById("answers-table");
  table.innerHTML = "";

  for (let i = 0; i < state.data[state.card].A.length; i++) {
    const txt = state.data[state.card].A[i];
...
function updateTransform() {
  ...
  const ry = 'rotateY(' + floor((state.rotation + rotationModifier) * 180) + 'deg)';
